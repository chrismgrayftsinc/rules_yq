...
exports_files(["yq-{os}"])
yq_toolchain(name = "rules_yq_binary_toolchain", yq = ":yq-{os}", version = "{version}", os = "{os}")
...
# Taken mostly from https://github.com/aspect-build/bazel-lib/blob/main/lib/private/yq_toolchain.bzl
def _yq_toolchains_repo_impl(ctx):
    starlark_content = """# @generated by @aspect_bazel_lib//lib/private:yq_toolchain.bzl

# Forward all the providers
def _resolved_toolchain_impl(ctx):
    toolchain_info = ctx.toolchains["@rules_yq//yq:toolchain"]
    return [
        toolchain_info,
...
resolved_toolchain = rule(
    ...
    toolchains = ["@rules_yq//yq:toolchain"],
    incompatible_use_toolchain_transition = True,
)
"""
    ctx.file("defs.bzl", starlark_content)
...
_yq_toolchains_repo = repository_rule(
    _yq_toolchains_repo_impl,
)


def register_yq_toolchain(version, register = True):
    ...
    _yq_toolchains_repo(name = "yq_toolchains")

def rules_yq_toolchains(version = YQ_DEFAULT_VERSION, register = True):
    ...
    register_yq_toolchain(version = version, register = register)
